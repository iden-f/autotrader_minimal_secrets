...
GitHub Actions already serialises scheduled runs with a `concurrency` group,
but nothing stops a manual `python -m autotrader run` colliding with a cron
job on a machine you run it on yourself. Two runs interleaving their writes to
state.json is exactly how duplicate notifications happen.
"""
