...
The previous bot identified itself as ``AutoTraderBot/1.0``, had no retries and
treated any non-200 as a fatal crash.  That combination is what killed it: one
bad response ended the run before state was saved.  Everything here is built so
that a failure is *reported* rather than fatal.
"""
...
# A plain, current desktop Chrome UA.  Nothing clever - just not a UA that
# announces itself as a scraper.
DEFAULT_USER_AGENT = (
