...
The search-results markup was inferred rather than captured, so the first real
run against the live site is the first time anybody finds out whether it works.
This decides that automatically: a parse that looks wrong stops the run before
anything is recorded, rather than quietly filling state with garbage that then
has to be unpicked.
"""
...
def report_markdown(assessments: list[Assessment], *, ok: bool) -> str:
    """A human-readable report for the run log, an artifact and the job summary."""
