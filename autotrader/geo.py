...
The 2026 platform ignores the ``prx``/``loc`` parameters a pasted link carries,
so a search that says "near V6N 3B5" happily returns cars in Ontario, Alberta
and Quebec. Nothing about the request fails; the constraint is simply dropped.
This is what lets the bot put it back.

Coordinates are to about a kilometre, which is far more precision than a
"would I drive to see it" radius needs. The table covers the places Canadian
listings actually name; anything it does not know is handled by never
excluding a car we cannot place - see ``too_far``.
"""
...
FSA = {
    "V6N": (49.264, -123.196),   # Vancouver, Point Grey
    "V6C": (49.287, -123.117),   # Vancouver, downtown
    "V5K": (49.281, -123.030),   # Vancouver, Hastings-Sunrise
    "M5V": (43.645, -79.395),    # Toronto, entertainment district
    "M4W": (43.674, -79.383),    # Toronto, Rosedale
    "T2P": (51.047, -114.070),   # Calgary, downtown
    "H3B": (45.502, -73.570),    # Montreal, downtown
}

# City -> (lat, lon), keyed by province. Built from the places Canadian
# listings name, plus everywhere else of any size, so an unfamiliar town is
...
def distance_km(a: tuple[float, float], b: tuple[float, float]) -> float:
    """Great-circle distance. Road distance is longer, which is the safe way
    round for a filter: it never excludes something that is actually closer."""
...
def locate_reference(text: str) -> tuple[float, float] | None:
    """Where "V6N 3B5", "Vancouver, BC" or "Vancouver" is."""
...
def nearest_in(province: str, point: tuple[float, float]) -> float | None:
    ...
    This is what lets an unfamiliar town still be excluded honestly: if the
    closest place we know of in Ontario is 3,000 km away, an Ontario town we
    have never heard of is not within 250 km either.
    """
...
def too_far(city: str, province: str, reference: tuple[float, float],
    ...
    A car we cannot place is never excluded. Hiding a match because a town is
    missing from a table would be the worst kind of failure this bot can have,
    and the table is the thing most likely to be incomplete.
    """
    if radius_km <= 0:
        return False, None
