...
hunting that car any more" - which is how this arrived, with 255 BMW M5s in a
dashboard pointed at three entirely different cars.
"""
...
def car(lid: str, search_id: str, **kw) -> Listing:
    base = dict(id=lid, url=f"https://www.autotrader.ca/a/x/19_{lid}_/",
                title=f"BMW M5 {lid}", search_id=search_id,
                price=90000, price_source="detail")
    base.update(kw)
    return Listing(**base)
...
def bench(tmp_path, monkeypatch):
    ...
    kept = cfg.add_search("https://www.autotrader.ca/cars/bmw/m3/?rcp=25", "keep")
    going = cfg.add_search("https://www.autotrader.ca/cars/bmw/m5/?rcp=25", "going")
    cfg.save()
...
class TestForgetting:
    ...
    def test_a_v1_import_with_no_search_belongs_to_nothing(self, bench):
        state = bench.state()
        state.listings["legacy"] = {"id": "legacy", "status": "archived",
                                    "notified": True, "price_history": []}
        state.forget_listings({bench.keep})
        assert "legacy" not in state.listings
