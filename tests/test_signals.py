...
def listing(price=90000, **over) -> Listing:
    data = {
        ...
        "title": "2021 BMW M5 Competition", "price": price,
        "url": "https://www.autotrader.ca/a/bmw/m5/x/y/2b555f88/",
        "year": 2021, "make": "BMW", "model": "M5",
...
class TestCrossingBackIntoYourRules:
    def test_a_hidden_car_that_qualifies_is_not_a_new_listing(self):
        st = fresh()
        st.record(listing(price=139888), filtered=True,
                  filter_reason="price $139,888 above maximum $100,000")
        change = st.record(listing(price=95000), filtered=False)
    ...
    def test_a_car_admitted_by_a_rule_change_rather_than_a_price_change(self):
        ...
        st = fresh()
        st.record(listing(price=105000), filtered=True,
                  filter_reason="price $105,000 above maximum $100,000")
        change = st.record(listing(price=105000), filtered=False)
    ...
    def test_it_remembers_which_rule_had_been_hiding_it(self):
        ...
        st.record(listing(price=105000), filtered=True,
                  filter_reason="price $105,000 above maximum $100,000")
        st.record(listing(price=105000), filtered=False)
    ...
    def test_a_car_that_goes_from_hidden_to_hidden_does_not_either(self):
        ...
        st = fresh()
        st.record(listing(price=139888), filtered=True, filter_reason="too dear")
        change = st.record(listing(price=130000), filtered=True,
