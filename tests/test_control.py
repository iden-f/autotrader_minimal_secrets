...
* nothing is trusted. The body is attacker-controlled where issues are on -
  anyone can open one - so every value is parsed and bounded.
"""
    ...
    data = {
        ...
        "searches": [
            {"id": "alpha", "name": "Alpha", "enabled": True,
             "url": "https://www.autotrader.ca/cars/bmw/m5/",
...
def fenced(payload) -> str:
    return ("Some prose a person typed.\n\n```autotrader\n"
            + json.dumps(payload) + "\n```\n\nMore prose.")


class TestFindingTheInstruction:
    def test_an_issue_body_with_prose_around_it(self):
        items = control.parse(fenced([{"action": "set-rule", "rule": "max_price",
                                       "value": 5}]))
        assert items[0]["rule"] == "max_price"

    def test_a_committed_control_file_is_bare_json(self):
        """The transport that exists on every repository.

        Issues can be switched off per repository, and on the one this was
        built for they are: creating one answers 410. A file committed through
        GitHub's web editor needs no repository setting at all.
        """
    ...
    def test_prose_with_no_instruction_says_so(self):
        ...
        assert "autotrader" in str(exc.value)
...
class TestApplyingRules:
    def test_a_rule_on_one_search(self):
        ...
        assert cfg.data["searches"][0]["filters"]["max_price"] == 120000
    ...
    def test_nothing_is_written_when_any_instruction_is_refused(self):
        ...
        assert not out.changed
        assert cfg.data["searches"][0]["filters"]["max_price"] == 100000
...
class TestSearches:
    def test_adding_one(self):
        ...
        out = control.apply(cfg, st, [{
            ...
            "url": "https://www.autotrader.ca/cars/bmw/m3/"}])
...
class TestNothingHereTrustsTheText:
    """Where issues are enabled, this body is written by anyone on the internet."""
...
class TestTheRuleAPersonIsMostLikelyToChange:
    ...
    @pytest.mark.parametrize("place", [
        "V6N 3B5", "v6n3b5", "V6N", "Vancouver, BC", "Toronto",
    ...
    def test_clearing_it_is_a_real_request(self):
        cfg, st = cfg_with(), state_with()
        cfg.data["searches"][0]["filters"]["near"] = "V6N 3B5"
        control.apply(cfg, st, [{"action": "set-rule", "search": "Alpha",
