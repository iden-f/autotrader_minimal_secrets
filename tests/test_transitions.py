...
def page(cars: list[dict]) -> str:
    ...
    for i, car in enumerate(cars, 1):
        item = {
            "@type": "Vehicle",
            "name": car.get("name", f"{car.get('year', 2022)} BMW M5"),
            "url": f"https://www.autotrader.ca/offers/bmw-m5-{uuid_for(car['id'])}",
            "vehicleModelDate": str(car.get("year", 2022)),
            "model": "M5", "brand": {"@type": "Brand", "name": "BMW"},
        ...
        if car.get("city"):
            item["availableAtOrFrom"] = {
                ...
                "address": {"@type": "PostalAddress",
                            ...
                            "addressRegion": car.get("region", "BC")}}
...
class Bench:
    ...
    def __init__(self, tmp_path, monkeypatch, filters=None, searches=None):
        ...
        for url, name in (searches or [(BASE + "?rcp=25", "M5")]):
...
def assert_books_balance(report, state):
    ...
    for lid, e in state.listings.items():
        if e.get("imported_from") or e.get("migrated_from"):
            continue
...
class TestHiddenAndBack:
    ...
    def test_a_car_that_stops_being_hidden_is_announced_and_loses_the_reason(self, bench):
        b = bench(filters={"max_price": 100000})
        r1 = b.check([{"id": "a", "price": 139888}])
    ...
    def test_and_back_again(self, bench):
        b = bench(filters={"max_price": 100000})
        b.check([{"id": "a", "price": 88000}])
        r = b.check([{"id": "a", "price": 139888}])
        assert_books_balance(r, b.state())
        assert b.entry("a")["filtered"]
        assert b.entry("a")["quiet_reason"].startswith("hidden by your rules")

    def test_hidden_then_no_price_at_all(self, bench):
        """The live failure: no price is not a rejection, so it unfilters."""
        b = bench(filters={"max_price": 100000, "require_price": True})
        b.check([{"id": "a", "price": 139888}])
...
class TestTheWholeCycleAtOnce:
    def test_a_car_through_every_state_keeps_its_books(self, bench):
        ...
        b = bench(filters={"max_price": 100000})
        steps = [
            ([{"id": "a", "price": 139888}, {"id": "z", "price": 50000}], "hidden"),
            ([{"id": "a", "price": 88000}, {"id": "z", "price": 50000}], "visible"),
            ([{"id": "a", "price": None}, {"id": "z", "price": 50000}], "no price"),
            ([{"id": "a", "price": 84000}, {"id": "z", "price": 50000}], "priced again"),
            ([{"id": "z", "price": 50000}], "absent once"),
            ([{"id": "z", "price": 50000}], "absent twice"),
            ([{"id": "a", "price": 82000}, {"id": "z", "price": 50000}], "back"),
        ]
