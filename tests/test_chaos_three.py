...
class TestTheAllowanceRunsOut:
    ...
    def test_an_exhausted_allowance_does_not_stop_an_exempt_repository(self, bench):
        """Proved on 13 September: the account's 3,000 minutes hit 100% and
        this repository's scheduled runs kept succeeding, because a public
        repository on a standard runner is not billed."""
        state = bench.state()
        now = clock.now()
        month = f"{now.year:04d}-{now.month:02d}"
        state.data["actions"] = {
...
class TestATokenIsRevoked:
    ...
    the page, answering an issue) and never the check.
    """
    ...
    def test_a_failed_publish_cannot_lose_the_check(self):
        ...
        checked = text.index("- name: Check for new listings")
        saved = text.index("- name: Save results")
